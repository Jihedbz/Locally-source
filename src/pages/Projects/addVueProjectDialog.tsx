...
export function AddVueProjectDialog() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await tauriCommands.createVueProject(projectName)
      ...
      show('success', 'Vue project created successfully!')
    ...
    <div className="mx-auto max-w-3xl px-6 py-8">
      <div className="rounded-3xl border border-border/70 bg-background/90 p-8 shadow-sm">
        <div className="space-y-3 text-center">
          <p className="text-sm uppercase tracking-[0.2em] text-muted-foreground">Vue wizard</p>
          <h1 className="text-3xl font-semibold tracking-tight">Create a new Vue project</h1>
          <p className="mx-auto max-w-2xl text-sm text-muted-foreground">
            Create a TypeScript Vue workspace and add it to your local project library.
          </p>
        </div>
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          ...
          <div className="rounded-2xl border border-border/60 bg-muted/50 p-4 text-sm text-muted-foreground">
            Vue, Vite, and TypeScript will be configured automatically.
          </div>
          <Button type="submit" size="lg" disabled={isCreating}>
            {isCreating ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
            {isCreating ? 'Creating Vue project...' : 'Create Vue project'}
          </Button>
        </form>
      </div>
    </div>
...
export default AddVueProjectDialog
